module CorelightLogs;

export {
        ## If you would like to disable your default logs and only log the
        ## "Corelight" format of logs set this to `T`.  By default this setting
        ## will continue logging your logs in whatever format you specified
        ## and also log them with the "corelight_" prefix and all of the
        ## associated Corelight settings.
        const CorelightLogs::disable_default_logs = F &redef;
        ...
        ## with the following scheme: `corelight_<path>.<num>.log`.  So, the
        ## first conn log would be named: `corelight_conn.1.log`.
        const CorelightLogs::extra_files = 4 &redef;

        ## A rotation interval specifically for the Corelight logs.  This is
        ...
        const CorelightLogs::rotation_interval = 15mins &redef;
}

type CorelightLogExtension: record {
...
function add_corelight_log_extension(path: string): CorelightLogExtension
        {
        return CorelightLogExtension($path        = sub(path, /^corelight_/, ""),
...
event zeek_init() &priority=-1000
        ...
        for ( stream in Log::active_streams )
                ...
                if ( CorelightLogs::disable_default_logs )
                        filt$name = "default";
                else
                        filt$name = "corelight-json";

                filt$path = "corelight_" + filt$path;
                filt$writer = Log::WRITER_ASCII;
                filt$postprocessor = rotate_logs;
                filt$interv = rotation_interval;

                filt$ext_func = add_corelight_log_extension;
                filt$ext_prefix = "_";
